...
from GearMess.JIM.JIMs import Jim, MessageConverter
from GearMess.JIM.jim_config import *
from GearMess.client_src.client_storage_handler import ClientStorageHandler
from GearMess.crypto.crypto import *
...
class Client:
    ...
    # def sync(self):
    #     history_list = []
    #     msg = self.message_maker.create(action=SYNC)
    #     msg = self.converter(msg)
    #     self.soc.send(msg)
    #     resp = self.soc.recv(BUF_SIZE)
    #     resp = self.converter(resp)
    #     for _ in range(resp[QUANTITY]):
    #         mess = self.soc.recv(BUF_SIZE)
    #         mess = self.converter(mess)
